...
def eval(model, dataloader_eval, post_process=False):
    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        with torch.no_grad():
            ...
            pred_depth, uncertainty_maps = model(image)
            a_map = uncertainty_maps[1]
            # print("a_map size is", a_map.size())
            b_map = uncertainty_maps[2]
        ...
        u_map = u_map.cpu().numpy().squeeze().squeeze()
        ...
        diff_abs_coff[diff_abs_coff==np.inf] = 0
        diff_abs_coff[diff_abs_coff==np.nan] = 80
        eval_measures[:9] += torch.tensor(measures).cuda()
        eval_measures[9] += 1
        
        try:
            pearsonr_coff, _ = pearsonr(u_map.flatten(), diff_abs_coff.flatten())
            print("pearsonr_coff:", pearsonr_coff)
    ...
    print(np.mean(np.array(CC_pearson)))
