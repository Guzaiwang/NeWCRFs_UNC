...
class TempCombLoss(nn.Module):
	...
	def forward(
		self,
		mean: Tensor, one_over_alpha: Tensor, beta: Tensor, target1: Tensor, target2: Tensor, mask, 
		T1: float, T2: float
	):
		##target1 is the base model output for identity mapping
		##target2 is the ground truth for the GenGauss loss
		l1 = self.L_l1(mean[mask], target1[mask])
		l2 = self.L_GenGauss(mean[mask], one_over_alpha[mask], beta[mask], target2[mask])
		l = T1*l1 + T2*l2
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            depth, uncertainty_maps = model(image)
            depth_est = depth.clone()

            # depth_est = uncertainty_maps[1] + depth
            ...
            # print("depth_gt max min", depth_gt.max(), depth_gt.min())
            loss = silog_criterion(uncertainty_maps[0], uncertainty_maps[1], uncertainty_maps[2], depth_est, depth_gt, mask.to(torch.bool), T1=T1,T2=T2)
            loss.backward()
            ...
            if global_step and global_step % 2000 == 1999 and args.rank % ngpus_per_node == 0:
                model.eval()
                model_save_name = '/model-epoch{}-iter_{}'.format(epoch, global_step)
